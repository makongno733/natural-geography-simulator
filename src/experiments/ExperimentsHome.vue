<template>
  <div class="exp-home">
    ...
    <div class="exp-grid">
      <router-link v-for="category in categories" :key="category.id" :to="`/experiments/${category.id}`" :class="['exp-card', `exp-card-${category.id}`]">
...
.exp-filter { max-width: 900px; margin: 42px auto 0; padding: 20px; border: 1px solid var(--border); border-radius: var(--radius-card); background: var(--card-bg); }
...
.experiment-result { display: grid; gap: 4px; padding: 12px; border: 1px solid var(--border); border-radius: 6px; color: var(--text); text-decoration: none; background: var(--surface); }
