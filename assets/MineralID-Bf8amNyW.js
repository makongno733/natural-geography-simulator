const t=[{title:"观察颜色和光泽",content:"在自然光下观察矿物标本的直观属性——颜色和光泽。颜色是矿物对可见光选择性吸收的结果，可分为自色（矿物固有成分决定）、他色（杂质致色）和假色（表面氧化膜或干涉）。光泽描述矿物表面对光的反射能力，常见类型包括：金属光泽（如黄铁矿）、半金属光泽、金刚光泽（如金刚石）、玻璃光泽（如石英）、油脂光泽、珍珠光泽、丝绢光泽和土状光泽。",highlight:"颜色是初步鉴定线索但不可靠——同一种矿物可呈现不同颜色（如石英有紫水晶、烟水晶、蔷薇石英），不同矿物也可有相同颜色。光泽则相对稳定。",illustration:`<svg width="100%" height="auto" viewBox="0 0 500 300" xmlns="http://www.w3.org/2000/svg">
  <defs>
...
</svg>`},{title:"条痕测试",content:"将矿物在未上釉的白色瓷板（条痕板）上划擦，观察留下的粉末颜色。条痕色是矿物粉末的颜色，比矿物块体的颜色更可靠，因为粉末消除了表面杂质和晶体缺陷的影响。操作要点：手持矿物，以一定力度在瓷板上划一道约 2-3 cm 的线，观察粉末颜色。注意：条痕板硬度约 7，硬度大于 7 的矿物无法在瓷板上留痕——需研磨后观察粉末。",highlight:"条痕色是矿物的可靠鉴定特征。例如赤铁矿无论外表是钢灰还是红褐色，条痕恒为樱红色；黄铁矿外表金黄但条痕为绿黑色，可与金（条痕金黄色）区分。",illustration:`<svg width="100%" height="auto" viewBox="0 0 500 300" xmlns="http://www.w3.org/2000/svg">
  <defs>
...
</svg>`},{title:"莫氏硬度测试",content:`莫氏硬度是矿物抵抗刻划的能力，用 10 种标准矿物定义从 1 到 10 的相对硬度等级。测试方法：用已知硬度的物体（或标准矿物）刻划未知矿物，观察是否留下划痕。

常见参照物硬度：指甲 ~2.5、铜币 ~3.0、铁钉 ~4.5、小刀/玻璃 ~5.5、钢锉 ~6.5、石英 ~7。

莫氏硬度计标准矿物：1-滑石、2-石膏、3-方解石、4-萤石、5-磷灰石、6-正长石、7-石英（水晶）、8-黄玉、9-刚玉、10-金刚石。

操作：由软到硬依次测试，直到找到能刻划矿物和不能被矿物刻划的相邻参照物，确定矿物硬度范围。`,highlight:"莫氏硬度是相对硬度而非绝对硬度——金刚石（10）的绝对硬度是滑石（1）的约 1600 倍。野外常用指甲、铜币、小刀作为快速判据。",illustration:`<svg width="100%" height="auto" viewBox="0 0 500 300" xmlns="http://www.w3.org/2000/svg">
  <defs>
...
</svg>`},{title:"解理观察",content:`解理是矿物受外力作用时沿一定结晶学方向裂开成光滑平面的性质。观察解理需要放大镜（10x），注意解理的方向数、夹角和完整程度。
...
- 无解理：如石英，断口呈贝壳状`,highlight:"解理方向和夹角是鉴定相似矿物的关键——辉石和角闪石在颜色、硬度上相近，但解理夹角不同（辉石近 90°，角闪石约 56°），是最可靠的区分特征。",illustration:`<svg width="100%" height="auto" viewBox="0 0 500 300" xmlns="http://www.w3.org/2000/svg">
  <defs>
...
</svg>`},{title:"酸反应测试",content:`碳酸盐矿物（方解石 CaCO₃、白云石 CaMg(CO₃)₂ 等）遇稀盐酸（通常 5%-10% HCl）会产生 CO₂ 气泡。这是鉴定碳酸盐矿物的快速方法。

操作步骤：在矿物新鲜表面滴一滴稀盐酸，观察气泡反应。方解石在冷稀盐酸中剧烈起泡；白云石在冷稀盐酸中反应微弱，需加热或使用粉末；菱镁矿类似。

注意：稀盐酸有腐蚀性，实验后需用清水冲洗标本；避免将酸滴在成品晶簇或珍贵标本上；野外可用稀醋酸代替。`,highlight:"CaCO₃ + 2HCl → CaCl₂ + H₂O + CO₂↑。方解石（石灰岩的主要成分）在冷稀盐酸中剧烈起泡是最典型的鉴定反应，地质工作者常用此法在野外快速识别石灰岩和白云岩。",illustration:`<svg width="100%" height="auto" viewBox="0 0 500 300" xmlns="http://www.w3.org/2000/svg">
  <defs>
...
</svg>`}],e={name:"MineralID",steps:t};export{e as default};
