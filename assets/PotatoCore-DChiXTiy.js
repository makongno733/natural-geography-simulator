const t=[{title:"布置勘探网格",content:`在地表按一定间距布置勘探网格，标记采样点位。网格间距根据勘探精度要求确定——区域性普查可用较大间距（如 200-500 m），详查阶段需加密至 50-100 m。

本实验中，将土豆视为"地下岩体"，在其表面用牙签或标记笔画出网格线，纵横各 4-5 条，形成 16-25 个网格交点。每个交点即为一个钻孔位置。记录每个孔位的坐标（行号、列号），建立采样定位系统。`,highlight:"勘探网格是系统化地质勘探的基础——网格越密，对地下构造的空间分辨率越高，但成本也越大。实际矿产勘探中，网格设计需在精度与成本之间权衡。",illustration:`<svg width="100%" height="auto" viewBox="0 0 500 300" xmlns="http://www.w3.org/2000/svg">
  <defs>
...
</svg>`},{title:"钻取岩心样本",content:`在标记的每个网格点，用透明吸管（模拟岩心钻机）垂直插入土豆中，旋转并向下推进，获取一段完整的土豆柱作为"岩心样本"。
...
5. 如果插入深度不够或样本破碎，需在同一位置重新取样`,highlight:"实际岩心钻探中，钻头内装有岩心管，钻进时岩心进入管内，每钻进一定深度（通常 1.5-3 m）提钻取出岩心。岩心采取率（取出岩心长度/钻进深度）是衡量钻探质量的重要指标，一般要求 > 85%。",illustration:`<svg width="100%" height="auto" viewBox="0 0 500 300" xmlns="http://www.w3.org/2000/svg">
  <defs>
...
</svg>`},{title:"记录岩心数据",content:`对每个孔位取出的岩心样本进行详细记录。记录内容包括：
...
将所有孔位数据整理成表格，形成系统的钻孔编录。`,highlight:"岩心编录是地质勘探的核心环节。地质学家通过岩心判断岩性（岩石类型）、构造（断层、节理、褶皱）、矿化（矿石品位、蚀变类型）等关键信息。一块岩心就是一条垂直的地质时间线。",illustration:`<svg width="100%" height="auto" viewBox="0 0 500 300" xmlns="http://www.w3.org/2000/svg">
  <defs>
...
</svg>`},{title:"重建三维模型",content:`利用各钻孔的岩心数据，推断地下地质体的三维空间分布。
...
结论：钻孔数据本质上是三维地质体的一维"采样线"——通过多条平行采样线的空间插值，可以推断地下构造，这正是矿产勘探和地质建模的基本方法。`,highlight:"从一维岩心数据到三维地质模型，需要空间推理和地质知识。相邻钻孔的相似层位可以对比连接，形成地质剖面。这一过程在现代勘探中借助计算机软件（如 Leapfrog、Surpac）实现三维建模和储量估算。",illustration:`<svg width="100%" height="auto" viewBox="0 0 500 300" xmlns="http://www.w3.org/2000/svg">
  <defs>
...
</svg>`}],e={name:"PotatoCore",steps:t};export{e as default};
